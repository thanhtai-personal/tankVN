...
namespace TankVN
{
    ...
    class cell : VisibleGameEntity
    {
        ...
        public cell(Vector2 position, CellType type)
        {
            ...
            switch (type)
            {
                ...
                case CellType.protect:
                    ...
                    break;
                case CellType.Cell:
                    TextureMultiFrame temp4 = new TextureMultiFrame(GLOBAL.buicaytexture, 26, 26);
                    this.Model = new MySprite2D(temp4, 10000);                    
                    break;
                case CellType.Nuoc:
                    TextureMultiFrame temp3 = new TextureMultiFrame(GLOBAL.waterTt, 26, 26);
                    this.Model = new MySprite2D(temp3, 100);
                    this.Model.FristFrame = 1;
                    this.Model.LastFrame = 21;
                    break;
        ...
        public override void Update(GameTime gametime)
        {
                ...
                {               
                    ...
                    if (GLOBAL.vachampixel(new Rectangle((int)this.Model.Position.X, (int)this.Model.Position.Y, (int)this.Model.Width, this.Model.Height), datacolor, GLOBAL.Bullet[i].Position, GLOBAL.Bullet[i].Model.MultiFramelist.datacolor))
                    {
                        ...
                        if (GLOBAL.Bullet[i].Type==bullet.loaidan.player)
                            GLOBAL.ExplosionGachsound.Play(1f,1f,0f);
                        GLOBAL.explosionGach.Add(new MySprite2D(new TextureMultiFrame(GLOBAL.explosionGachTt, 64, 64), 15));
           ...
           if (type == CellType.Nuoc) this.Model.Update(gametime);
            //else 
