    ...
    class player : tank
    {
        ...
        public override void update(GameTime gameTime)
        {
            ...
            if (keyboardState.IsKeyDown(Keys.F))
            {
                ...
                {
                    GLOBAL.Bullet.Add(new bullet(new Rectangle(X, Y, 11, 11), bullet.loaidan.player, new Force(4, _Direction, 1)));
        ...
        public void Draw(int firstframe, int lastframe, object obj, Vector2 position, Color color, float rotation, Vector2 origin, float scale, float layerDepth)
        {
            ...
            for (int i = 0; i < GLOBAL.Bullet.Count; i++)
            {
                ...
                GLOBAL.Bullet[i].Draw(0, 3, obj, position, color, rotation, origin, scale, layerDepth);
            }
        }
    }
}
